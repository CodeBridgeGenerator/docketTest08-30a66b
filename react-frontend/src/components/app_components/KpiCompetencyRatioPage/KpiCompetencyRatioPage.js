...
import KpiCompetencyRatioDatatable from "./KpiCompetencyRatioDataTable";
...
import KpiCompetencyRatioFakerDialogComponent from "./KpiCompetencyRatioFakerDialogComponent";
import KpiCompetencyRatioSeederDialogComponent from "./KpiCompetencyRatioSeederDialogComponent";
import SortMenu from "../../../services/SortMenu";
import FilterMenu from "../../../services/FilterMenu";

const KpiCompetencyRatioPage = (props) => {
    ...
    const filename = "kpiCompetencyRatio";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("kpiCompetencyRatio");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "KpiCompetencyRatio", type: "error", message: error.message || "Failed get KpiCompetencyRatio" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("kpiCompetencyRatio").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "KpiCompetencyRatio", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("kpiCompetencyRatio").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "KpiCompetencyRatio",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/kpiCompetencyRatio/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>KpiCompetencyRatio </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="kpiCompetencyRatio-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="kpiCompetencyRatio-datatable">
                    <KpiCompetencyRatioDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <KpiCompetencyRatioEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <KpiCompetencyRatioCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <KpiCompetencyRatioFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <KpiCompetencyRatioSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(KpiCompetencyRatioPage);
