...
import kpiCompetencyRatioFakerFactory from "./kpiCompetencyRatioFakerFactory";
...
const KpiCompetencyRatioFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = kpiCompetencyRatioFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("kpiCompetencyRatio").create(elem));
...
export default connect(mapState, mapDispatch)(KpiCompetencyRatioFakerDialogComponent);
